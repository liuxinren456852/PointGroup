...
SAVE_ROOT = '/data2/helin/10.0_PointGroup/dataset/scannetv2/train/'
...
def f(fn):
    ...
    for i in range(pts.shape[0]):
        ...
        coords = pts[i] - pts[i].mean(0)
        sem_labels = gt_label[i].astype(np.int8)
        sem_labels -= ((sem_labels == np.zeros(sem_labels.shape)) * 100).astype(np.int8)
        colors = pts_rgb[i]
        instance_labels = np.zeros(gt_mask.shape[2])
        for j in range(gt_mask.shape[1]): # N
            instance_labels += gt_mask[i][j].astype(np.int8) * (j+1) # TODO:?

        torch.save((coords, colors, sem_labels, instance_labels.astype(np.int8)), SAVE_ROOT + save_name + f'_{i}.pth')
    # print(coords,'print\n', colors,'print\n', sem_labels,'print\n', instance_labels)
